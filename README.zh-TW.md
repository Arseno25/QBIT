...
**裝置設定選單**
長按觸控感測器進入設定選單。最上層有三個項目：**TIMER**、**GAME LIBRARY**、**SETTING**。單擊可捲動，長按可進入。在最上層雙擊可離開選單。選單若 10 秒無操作會自動關閉。**SETTING** 底下的選項須選擇 **[ SAVE ]** 後才會生效。

| 項目 | 說明 |
|---|---|
| TIMER | 倒數計時：設定時／分／秒後開始；時間到會播放鬧鈴（即使靜音也會響）。單擊可關閉鬧鈴，長按可回到設定畫面。 |
| GAME LIBRARY | 遊戲列表：**T-Rex Runner**（小恐龍）、**Flappy Bird**（像素鳥）。長按可進入遊戲；單擊可捲動。 |
| SETTING | 開關與時鐘格式（見下表）。 |
